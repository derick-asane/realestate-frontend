...
const UserProperty = () => {
  ...
  const PropertyCard = ({ property, isListView = false }) => {
    ...
    //addProperty to favorite

    const HandleAddFavorite = async (propertyId) => {
      console.log("add property id:", propertyId);
      const user = JSON.parse(sessionStorage.getItem("user"));
      try {
        const response = await axios.post(
          "http://localhost:3000/favorite/addfavorite",
          {
    ...
    const isFavorited = () => {
      return property.favorites.some((fav) => fav.userId === currentUser.id);
    };
